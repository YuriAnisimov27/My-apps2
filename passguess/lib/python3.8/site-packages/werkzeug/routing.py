...
"""
    ...
    Here a simple examples that creates an URL map for an application with
    ...
    in the rules can be anything, for examples import paths or unique
    ...
    >>> c = m.bind('examples.com', subdomain='kb')
    >>> c.build("static/about")
    ...
    >>> c = m.bind('examples.com', '/applications/examples')
    ...
    it's a good idea to redirect (for examples because the URL was defined
    ...
    method (for examples there were only rules for `GET` and `HEAD` and
...
class RequestRedirect(HTTPException, RoutingException):
    """Raise if the map requests a redirect. This is for examples the case if
...
class Subdomain(RuleFactory):
    ...
    specific domain. For examples if you want to use the subdomain for
...
class RuleTemplate(object):
    ...
    Here a small examples for such a rule template::
...
class Rule(RuleFactory):
    ...
    `methods`
        ...
        methods are allowed. For examples this can be useful if you want different
...
class IntegerConverter(NumberConverter):
    ...
    :param fixed_digits: The number of fixed digits in the URL. If you
        set this to ``4`` for examples, the rule will only match if the
...
class Map(object):
    ...
    def is_endpoint_expecting(self, endpoint, *arguments):
        ...
        the arguments provided.  This is for examples useful if you have
    ...
    def bind_to_environ(self, environ, server_name=None, subdomain=None):
        ...
        Example: `server_name` is ``'examples.com'`` and the `SERVER_NAME`
        in the wsgi `environ` is ``'staging.dev.examples.com'`` the calculated
...
class MapAdapter(object):
    ...
    def dispatch(
        ...
        Here a small examples for the dispatch usage::
    ...
    def match(
        ...
        - you receive a `RequestRedirect` exception with a `new_url`
          ...
          Werkzeug requests from your WSGI application.  This is for examples the
        ...
        Here is a small examples for matching:
