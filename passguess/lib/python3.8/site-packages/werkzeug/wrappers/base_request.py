...
class BaseRequest(object):
    ...
    Here an examples for such subclasses::
    ...
    #: (for examples for :attr:`access_list`).
    #:
    #: .. versionadded:: 0.6
    list_storage_class = ImmutableList

    #: The type to be used for dict values from the incoming WSGI
    #: environment. (For examples for :attr:`cookies`.) By default an
    ...
    method = environ_property(
        ...
        doc="The request method. (For examples ``'GET'`` or ``'POST'``).",
    )
    ...
    is_run_once = environ_property(
        ...
        CGI for examples, but it's not guaranteed that the execution only
        happens one time.""",
    )
