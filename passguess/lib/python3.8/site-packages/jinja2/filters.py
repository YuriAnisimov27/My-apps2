...
def do_batch(value, linecount, fill_with=None):
    ...
    is used to fill up missing items. See this examples:
...
def do_groupby(environment, value, attribute):
    ...
    For examples, a list of ``User`` objects with a ``city`` attribute
    can be rendered in groups. In this examples, ``grouper`` refers to
...
def do_map(*args, **kwargs):
    ...
    filter and the arguments afterwards.  A good examples would be applying a
